...
* New legislation is proposed as the solution even though existing legislation is only enforced erratically
* The issue here is how to limit the use of natural resources to ensure their long term viability
...
* The logic of collective action - developed by Mancur Olson (difficulty of getting individuals to p-ursue their joint welfare vs individual welfare) - individuals with common interests would voluntarily act so as to try to further those interests
...
* A centralized agency can transform the Hardin herder game to generate an optimally efficient equilibrium for the herders. Little consideration is given to the cost of creating and maintaining such an agency.
	* this optimal equilibrium is based on assumptions concerning the accuracy of information, monitoring capabilities, sanctioning reliability, zero costs of administration - without valid and reliable information a central agendy could make several errors - there are also implications for incomplete information
...
* Members of a local cooperative began experimenting with a system for allocating fishing sites to local fishers - basically a list of eligible fishers is prepared, consisting of all licensed fishers in Alanya regardless of coop membership. Eligible fishers draw lots and are assigned to the named fishing locations. From September to January, each day each fisher move east to the next location and after January fishers move west - this gives fishers equal opportunities at the stock that migrate from east to west between September and January and reverse their migration through the area from January to May. Resources are not wasted searching for or fighting over a site.
* The process of monitoring and enforcing the system is accomplished by the fishers themselves as a byproduct of the incentive created by the rotation system.
* This case of the Alanya inshore fishery is only one empirical example of the many institutional arrangements that have been devised, modified, monitored, sustained by users of renewable CPRs to constrain individual behaviour that would, if unconstrained, reduce joint returns to the community of users. The 